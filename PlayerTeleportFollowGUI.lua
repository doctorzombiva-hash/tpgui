--// Player Teleport + Follow GUI with in-game scaling
...
teleportButton.Size = UDim2.new(0.5, -15, 0, 38)
...
followButton.Size = UDim2.new(0.5, -15, 0, 38)
followButton.Position = UDim2.new(0.5, 5, 0, 274)
followButton.BackgroundColor3 = COLORS.surface2
...
statusLabel.Position = UDim2.fromOffset(10, 316)
...
local function stopFollow(message)
	following = false
...
local function moveLocalCharacterTo(targetCFrame)
	...
	-- PivotTo moves the whole character model consistently.
		...
		if player ~= LocalPlayer then
			...
			if filter == "" or string.find(combined, filter, 1, true) then
				...
				button.Activated:Connect(function()
					if selectedPlayer ~= player and following then
						stopFollow("Follow stopped: target changed")
					end
...
local function teleportToPlayer(player)
	...
	local destination = targetRoot.CFrame * TELEPORT_OFFSET

	-- Repeat for a few frames to reduce immediate local physics snap-back.
	-- A server-side anti-teleport system can still override this.
	for _ = 1, 6 do
...
local function startFollow()
	...
	statusLabel.Text = "Following " .. selectedPlayer.DisplayName
	...
	followConnection = RunService.Heartbeat:Connect(function()
		...
		if not selectedPlayer or not selectedPlayer.Parent then
			stopFollow("Target left the server")
			return
		end
		...
		local ok = moveLocalCharacterTo(targetRoot.CFrame * FOLLOW_OFFSET)

		if not ok then
			statusLabel.Text = "Waiting for your character..."
		end
	end)
end
